...
- **Dialog Windows & Confirmations**: All confirmation dialogs must use consistent UI modal components instead of browser alerts (`alert()`, `confirm()`, `prompt()`). For any data deletion operation, implement a two-step confirmation process: (1) Show confirmation dialog with clear messaging about the action and consequences, (2) Display success/error feedback through UI dialogs. Use Tailwind CSS styled modals with proper backdrop, clear messaging, and consistent button styling (Cancel/Confirm). Never use browser popups for user interactions.
