...
export async function GET(request: Request) {
  ...
  const target = `${BFF_BASE}/oidc/authorize${url.search}`;
  ...
  // Rewrite location header to use our callback route
  ...
  if (location) {
    ...
    if (host && (host.includes("replit.dev") || host.includes("repl.co"))) {
      ...
      responseHeaders.set("location", `${finalOrigin}/api/bff/oidc/callback${callbackUrl.search}`);
      ...
      const callbackUrl = new URL(location);
      responseHeaders.set("location", `${origin}/api/bff/oidc/callback${callbackUrl.search}`);
    }
  }
